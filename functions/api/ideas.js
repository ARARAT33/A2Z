...
function parseDb(raw){let data=typeof raw==='string'?JSON.parse(raw||'[]'):raw;if(!Array.isArray(data))data=Array.isArray(data?.ideas)?data.ideas:(Array.isArray(data?.items)?data.items:[]);return data}
async function loadRawDb(){const r=await fetch(RAW_DB,{cache:'no-store'});if(!r.ok)throw new Error('raw-db');return parseDb(await r.text())}
async function getWriteFile(env){const r=await fetch(API,{headers:ghHeaders(env),cache:'no-store'});if(!r.ok)throw new Error('github-api');return await r.json()}
export async function onRequestGet(){try{const data=await loadRawDb();const today=data.filter(x=>isToday(x.created)).length;return json({dailyLimit:DAILY_LIMIT,publishedToday:today,remaining:Math.max(0,DAILY_LIMIT-today),hourLimit:HOUR_LIMIT,userDailyLimit:USER_DAY_LIMIT,available:today<DAILY_LIMIT})}catch{return json({dailyLimit:DAILY_LIMIT,publishedToday:0,remaining:DAILY_LIMIT,available:true},200)}}
export async function onRequestPost({request,env}){
 ...
 for(let attempt=0;attempt<3;attempt++){
  ...
  let file;try{file=await getWriteFile(env)}catch{return json({error:'Database write access is unavailable. Check the GitHub token permissions for AWEArchiveDB.'},502)}
